...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep V12 semantic-only exploration "
            "fusion weights against the label-free "
            "V11 shadow benchmark."
        )
    )
    ...
    for weight in weights:
        ...
        if baseline_initial is None:
            ...
            raise RuntimeError(
                "V12 changed the protected "
                "turn-1 ranking path during "
                "the sweep"
            )
