...
Local TF-IDF + TruncatedSVD (LSA)

Ranking

Deterministic constraint-aware reranking
...
Buying / Browsing + explicit intent override
...
Safe aggregate-profile tie-breaking for questions

External LLM/API required

No

Network required at scoring time
...
flowchart TD
    ...
    ST --> BC["Budget and hard-constraint parser"]
...
sequenceDiagram
    ...
    R->>R: BM25 plus optional semantic route
    R->>K: Candidate products
    K->>K: Apply budget and relevance ranking
    K-->>U: Ranked Top 10
    K->>Q: Candidate uncertainty
    Q-->>U: Ask next useful attribute if needed
    U->>S: Additional preference or correction
